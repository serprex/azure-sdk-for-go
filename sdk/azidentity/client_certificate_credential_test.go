...
func TestClientCertificateCredential_Live(t *testing.T) {
	...
	tests := []struct {
		...
		{"PEM", liveSP.pemPath}, {"PKCS12", liveSP.pfxPath},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.path == "" {
				t.Fatal("no certificate file specified")
			}
			cert, err := os.ReadFile(test.path)
			if err != nil {
				t.Fatalf(`failed to read cert "%s": %v`, test.path, err)
			}
			cred, err := NewClientCertificateCredential(tenantID, clientID, cert, nil)
			if err != nil {
